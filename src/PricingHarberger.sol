...
contract PricingHarberger is IPricing {
        ...
        returns (uint256, uint256)
    {
        ...
        if (lastUpdatedPrice <= minAnnualPrice) {
            ...
            int256 numYearsUntilMaxPrice = F.rawMulWad(
                1.4427e18,
                F.lambertW0Wad(
                    F.rawMulWad(69.314e18, F.expWad(F.rawMulWad(69.314e18, int256(lastUpdatedPrice))))
                )
            ...
            if (secondsSinceUpdate <= numSecondsUntilMaxPrice) {
                ...
                int256 numYearsUntilMinPrice = F.rawDivWad(
    ...
    function getIntegratedDecayPrice(uint256 p0, uint256 numSeconds) internal pure returns (uint256) {
        return uint256(
            F.rawMulWad(int256(p0), F.rawDivWad(getDecayMultiplier(numSeconds) - toWadUnsafe(1), F.lnWad(0.5e18)))
        );
    }
    ...
    function getDecayPrice(uint256 p0, uint256 numSeconds) internal pure returns (uint256) {
        return uint256(F.rawMulWad(int256(p0), getDecayMultiplier(numSeconds)));
    }
    ...
    function getDecayMultiplier(uint256 numSeconds) internal pure returns (int256) {
        return F.expWad(F.lnWad(0.5e18) * int256(numSeconds) / int256(SECONDS_IN_YEAR));
    }
    ...
    function getPriceAfterBid(uint256 p0, uint256 pBid, uint256 bidLengthInSeconds) internal pure returns (uint256) {
        ...
        return p0 + uint256(F.rawMulWad(int256(pBid - p0), F.rawMulWad(wadMonths, wadMonths)));
    }
}
